...
public class YeePayInvestNofreezeOperation extends
	...
	public TrusteeshipOperation createOperation(Invest invest, FacesContext fc)
		...
		/**2.0的端口
		 // 商户编号:商户在易宝唯一标识
				...
				// 商户平台会员标识:会员在商户平台唯一标识
				content.append("<platformUserNo>" + invest.getUser().getId() + "</platformUserNo>");
				// 请求流水号:接入方必须保证参数内的 requestNo 全局唯一，并且需要保存，留待后续业务使用
				content.append("<requestNo>" + YeePayConstants.RequestNoPre.INVEST + invest.getId() + "</requestNo>");
				
				//出款人用户类型，目前只支持传入 MEMBER 
				content.append("<userType>MEMBER</userType>");
				//根据业务的不同，需要传入不同的值，见【业务类型】。并参考下面的详细信息 
				content.append("<bizType>TENDER</bizType>");
				...
				//转入金额
					content.append("<amount>" + invest.getMoney() + "</amount>");
					//用户类型, 见【用户类型】
					content.append("<targetUserType>MEMBER</targetUserType>");
					//平台用户编号
					content.append("<targetPlatformUserNo>" + invest.getLoan().getUser().getId() + "</targetPlatformUserNo>");
					//资金明细业务类型。根据业务的不同，需要传入不同的值，见【业务类型】
					content.append("<bizType>TENDER</bizType>");
				content.append("</detail>");
				//业务扩展属性，根据业务类型的不同，需要传入不同的参数。
				//content.append("<extend>" + sdf.format(date) + "</extend>");
				// 页面回跳URL
				content.append("<callbackUrl>" + YeePayConstants.ResponseWebUrl.PRE_RESPONSE_URL + YeePayConstants.OperationType.INVEST + "</callbackUrl>");
				// 服务器通知 URL:服务器通知 URL
				content.append("<notifyUrl>" + YeePayConstants.ResponseS2SUrl.PRE_RESPONSE_URL + YeePayConstants.OperationType.INVEST + "</notifyUrl>");
				// 提现金额:如果传入此，将使用该金额提现且用户将不可更改
				content.append("</request>");
	...
	public void cancel(TrusteeshipOperation to, Invest invest) {
		...
		content.append("<requestNo>" + YeePayConstants.RequestNoPre.INVEST
				+ invest.getId() + "</requestNo>");
		content.append("</request>");
		log.debug("464"+content.toString());
