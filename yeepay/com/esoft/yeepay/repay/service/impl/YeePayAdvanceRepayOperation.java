...
public class YeePayAdvanceRepayOperation extends
	...
	public TrusteeshipOperation createOperation(Loan la, FacesContext fc)
		...
		if (log.isDebugEnabled()) {
			...
			//log.debug("feeToInvestor: "+feeToInvestor);	 取消给投资人罚金  		
		}
		...
		// 余额不足，抛异常
		// 按比例分给投资人和系统（默认优先给投资人，剩下的给系统，以防止提转差额的出现） -----不需要了，取消给投资人的罚金
		...
		// 更改投资的还款信息
		//double feeToInvestorTemp = 0;//feeToInvestor 给投资人的罚金取消
		//***********原本按比例分给投资人和系统 现在修改******************************
...
//			if (ir.getCorpus() != 0 && ir.getCorpus() != null) {
				...
				//获取剩余的本金，和下个月的利息，将下个月利息作为罚金给平台，剩余所以本金还给投资人 *******
				irRepayMoney = ArithUtil.add(ir.getCorpus(), feeAll, feeToSystem);
				irFree = ArithUtil.add(feeAll, feeToSystem);
