...
import com.esoft.yeepay.user.service.impl.YeePayUserMobileOperation;
...
public class TrusteeshipHome {
	 ...
	 */
	@Resource
	private YeePayUserMobileOperation yeePayUserMobileOperation;
	...
	public String handleWebReturn() {
		...
		}else if(YeePayConstants.OperationType.UPDATE_MOBILE
				.equals(this.operationType)){
	...
	public String withdrawCash() {
		...
		try {
			...
			if("0".equals(freezeMoney)){
				yeePayWithdrawNofreezeOperation.receiveOperationPostCallback(FacesUtil.getHttpServletRequest());
			} else {
				//yeePayWithdrawOperation.receiveOperationPostCallback(FacesUtil.getHttpServletRequest());
	...
	public void handleS2SWebReturn() {
		...
		if (YeePayConstants.OperationType.CREATE_ACCOUNT
			...
			openCorpAccountS2S();
		}else if(YeePayConstants.OperationType.UPDATE_MOBILE.equals(this.operationType)){
		//20150824修改手机 号
			this.receiveChangeMobileS2S();
		}else if(YeePayConstants.OperationType.RECHARGE2.equals(this.operationType)){
			this.experienceinvesRechargeS2S();
		}else if(YeePayConstants.OperationType.RECHARGE3
				.equals(this.operationType)){
			this.sendPlatformTransfer2S2SCallback();
		}
	...
	private void sendPlatformTransfer2S2SCallback(){
		yeePayRecheckOperation.sendPlatformTransfer2S2SCallback(FacesUtil.getHttpServletRequest(),
				FacesUtil.getHttpServletResponse());
	}
	...
	public String changeMobile(){
		try {
			yeePayUserMobileOperation.receiveOperationPostCallback(FacesUtil.getHttpServletRequest());
			// 给提示:修改绑定手机成功
			FacesUtil.addInfoMessage("手机绑定成功");
		} catch (TrusteeshipReturnException e) {
			FacesUtil.addInfoMessage("手机绑定失败：" + e.getMessage());
		}
		// 跳转到个人中心
		return "pretty:userCenter";
	}
	 ...
	 * @return
	 */
	public void receiveChangeMobileS2S(){
		yeePayUserMobileOperation.receiveOperationS2SCallback(FacesUtil.getHttpServletRequest(), FacesUtil.getHttpServletResponse());
	}
	
	/**
	 * 接收直接打款响应信息
	 * @author donghaocheng20151230
	 * @return
	 */
	public void experienceinvesRechargeS2S(){
		yeePayNormalRepayOperation.experienceinvesRechargeS2SCallback(FacesUtil.getHttpServletRequest(), FacesUtil.getHttpServletResponse());
	}
