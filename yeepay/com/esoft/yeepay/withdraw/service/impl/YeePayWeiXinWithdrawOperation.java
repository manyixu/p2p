...
@Service("yeePayWeiXinWithdrawOperation")
public class YeePayWeiXinWithdrawOperation extends
	...
	public TrusteeshipOperation createOperation(WithdrawCash withdrawCash,
		...
		log.debug("---------->>提现模式1："+withdrawCash.getWithdrawType());
		log.debug("---------->>提现模式2："+wc.getWithdrawType());
		//提现模式:如选择加急提现，此字段必须填写“URGENT”
		if ("1".equals(withdrawCash.getWithdrawType())) {//普通提现
			content.append("<withdrawType>"+YeePayConstants.WithdrawWay.NORMAL + "</withdrawType>");
		}
		if ("2".equals(withdrawCash.getWithdrawType())) {//快速提现
			content.append("<withdrawType>"+YeePayConstants.WithdrawWay.URGENT + "</withdrawType>");
		}
		// 费率模式:见费率模式
		// yee手续费收取方:1：平台支付 2：提现方支付
		String feeType = YeePayConstants.WithdrawYeeFee.USERPAY;
		try {
