...
public class UserBillListAdmin extends EntityQuery<UserBill>{

	
	private static final String COUNT_HQL = "select count(distinct userBill) from UserBill userBill left join userBill.user.roles roles ";
	private static final String HQL = "select distinct userBill from UserBill userBill left join userBill.user.roles roles ";
	...
	public UserBillListAdmin() {
		setCountHql(COUNT_HQL);
		setHql(HQL);
		final String[] RESTRICTIONS = { "id like #{userBillListAdmin.example.id}",
				"userBill.user.id like #{userBillListAdmin.example.user.id}",
				"userBill.type like #{userBillListAdmin.example.type}",
				"userBill.typeInfo like #{userBillListAdmin.example.typeInfo}",
				"userBill.time >= #{userBillListAdmin.startTime}",
				"userBill.time <= #{userBillListAdmin.endTime}",
				"roles.id = #{userBillListAdmin.roleId}"};
