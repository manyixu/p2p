...
///////////////////////////////////////////通过邮箱找回密码--结束////////////////////////////////////
	...
	//微信手机找回密码第一步
	public void weixinFindPwdByMobile1(){
		try {
			authService.verifyAuthInfo(getInstance().getId(), getInstance().getMobileNumber(),
					authCode,
					CommonConstants.AuthInfoType.FIND_LOGIN_PASSWORD_BY_MOBILE);
	...
	//微信手机找回密码第二步修改密码
	public String weixinFindPwdByMobile2() {
		try {
			User user = this.getInstance();
			userService.modifyPassword(user.getId(), newPassword);
			FacesUtil.addInfoMessage("修改密码成功！");
			...
			return "pretty:weixinapp_loginweixin";
		} catch (UserNotFoundException e) {
			String message = "对不起，手机验证失败！";
			FacesUtil.addErrorMessage(message);
			return null;
...
/////////////////////////////////修改绑定手机--开始////////////////////////////////////
	...
	/**
	 * 微信：：更改绑定手机号第一步 通过收到手机认证码验证用户当前手机
	 * 
	 * @return
	 */
	public void weixinCheckCurrentMobileNumber() {
		User user;
		try {
			user = userService.getUserById(loginUserInfo.getLoginUserId());
			authService.verifyAuthInfo(user.getId(), user.getMobileNumber(),
	...
	/**
	 * 微信：：更改绑定手机第4步 验证认证码并更改绑定手机
	 * 
	 * @return
	 */
	public String weixinChangeBindingMobileNumber() {
		User user;
		try {
			user = userService.getUserById(loginUserInfo.getLoginUserId());
			userService.bindingMobileNumber(user.getId(), newMobileNumber,
...
///////////////////////////////////////////修改绑定邮箱--开始////////////////////////////////////
	...
	/**
	 *微信找回密码第一步 给用户当前邮箱发送认证码
	 *@param jsCode 发送成功后执行的js代码
	 */
	
	public void sendWeiXinCurrentBindingEmail(String email, String jsCode){
		User l = userBO.getUserByEmail(email);
		if(l == null ){
			FacesUtil.getCurrentInstance().validationFailed();
		...
		Map<String, String> params = new HashMap<String, String>();
		params.put("time", DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD_HH_MM_SS_CN));
		params.put("authCode", authService.createAuthInfo(l.getId(), email, null, CommonConstants.AuthInfoType.CHANGE_BINDING_EMAIL).getAuthCode());
		// 发送手机验证码
		messageBO.sendEmail(getBaseService().get(UserMessageTemplate.class, MessageConstants.UserMessageNodeId.CHANGE_BINDING_EMAIL + "_email"), params, email);
		FacesUtil.addInfoMessage("验证码已经发送至邮箱！");
		RequestContext.getCurrentInstance().execute(jsCode);
	}
	...
	/**
	 * 微信：：更改绑定邮箱第二步 通过收到邮件认证码验证用户当前邮箱
	 */
	public void weixinCheckCurrentEmail(){
			try {
				User user=userService.getUserById(loginUserInfo.getLoginUserId());
				//验证认证码        CommonConstants.AuthInfoType.CHANGE_BINDING_EMAIL(类型) 修改绑定邮箱
				authService.verifyAuthInfo(user.getId(), user.getEmail(), authCode, CommonConstants.AuthInfoType.CHANGE_BINDING_EMAIL);
	...
	/**
	 * 微信：：找回密码第二步 通过收到邮件认证码验证用户当前邮箱
	 */
	public void weixinFindPwdCheckCurrentEmail(){
			try {
				authService.verifyAuthInfo(getInstance().getId(), getInstance().getEmail(), authCode, CommonConstants.AuthInfoType.CHANGE_BINDING_EMAIL);
				this.step = 2;
				this.authCode = null;
			} catch (NoMatchingObjectsException e) {
				FacesUtil.addErrorMessage("验证码输入有误！");
	...
	/**
	 * 微信更改绑定邮箱第四步 验证认证码并更改绑定邮箱
	 * 
	 * @return
	 */
	public String weixinChangeBindingEmail() {
		try {
			User user=userService.getUserById(loginUserInfo.getLoginUserId());
			userService.bindingEmail(user.getId(), newEmail, authCode);
			FacesUtil.addInfoMessage("绑定新邮箱成功！");
