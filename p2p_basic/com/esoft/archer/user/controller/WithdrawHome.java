...
public class WithdrawHome extends EntityHome<WithdrawCash> {
	...
	public boolean calculateFee() {
		...
		if (userBillService.getBalance(loginUserInfo.getLoginUserId())<fee+this.getInstance().getMoney()) {
			...
			Double urgentFee=2d;//快速提现手续费默认为2元Zhangwq20150825
			if (this.getInstance().getMoney()>4000) {
				urgentFee=this.getInstance().getMoney()*(Double.valueOf(YeePayConstants.UrgentWithdrawRate.URGENT_RATE));
				urgentFee=Double.valueOf(String.format("%.2f", urgentFee));				
			}
			this.getInstance().setUrgentFee(urgentFee);
			return true;
		}
	}
	/**
	...
	/**
	 * 提现时，增加快速提现
	 * @param sign 提现方式：1普通提现2快速提现
	 * @author Zhangwq20150817
	 * @return
	 */
