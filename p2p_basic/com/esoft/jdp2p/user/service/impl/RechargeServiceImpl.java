...
//		bcs.add(new RechargeBankCardImpl("CMB", "招商银行"));
	...
	@Override
	public Double getRechargeMoney(String sign, String userId, Date date) {
		String time=null;
		if ("D".equals(sign)) {
			time =DateUtil.DateToString(date, DateStyle.YYYY_MM_DD);
		} else if("M".equals(sign)) {
			time =DateUtil.DateToString(date, DateStyle.YYYY_MM);
		}
		String hql ="select sum(userBill.money) from UserBill userBill where userBill.user.id = '"+userId+"' and userBill.typeInfo='recharge_success'  and userBill.time like '"+time+"'";
		List<Double> rechargeList=ht.find(hql);
		Double rechargeMoney =0d;
		if (rechargeList.size()>0) {
			rechargeMoney=rechargeList.get(0);
		}
