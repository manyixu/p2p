...
public class UserMessageNodeList extends EntityQuery<UserMessageNode>{
	...
	public void changeStatus(UserMessageNode umw){
		//后台操作日志
		...
		if (umw.getStatus().equals(MessageConstants.UserMessageNodeStatus.OPEN)) {
			...
			operationLog.setOperationAction("用户消息发送节点-"+MessageConstants.UserMessageNodeStatus.CLOSED);
		} else {
			umw.setStatus(MessageConstants.UserMessageNodeStatus.OPEN);
			operationLog.setOperationAction("用户消息发送节点-"+MessageConstants.UserMessageNodeStatus.OPEN);
		} 
		getHt().update(umw);
