...
import com.esoft.jdp2p.loan.model.StartInterestAlertCount;
...
public class LoanServiceImpl implements LoanService {
		...
		try {
			...
			ubs.transferOutFromBalance(loan.getUser().getId(),
					...
					OperatorInfo.GIVE_MONEY_TO_BORROWER, "取出借款管理费, 借款ID："
							+ loan.getId());
			log.debug("Loan===>>借款管理费,用户ID："+loan.getUser().getId()+", 借款ID：" + loan.getId());
	...
	//起息提醒
	@Override
	public void startInterestAlert(){
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		String now = sdf.format(new Date());
		List<Loan> ls = ht.find("from Loan l where l.status =?",LoanConstants.RepayStatus.REPAYING);
		if (log.isInfoEnabled()) {
			log.info("repay alert start, size:" + ls.size());
		}
		//起息提醒
		for (Loan l : ls) {
			List<StartInterestAlertCount> siacs = ht.find("from StartInterestAlertCount s where s.loanId=?",l.getId());
			if(siacs!=null&&siacs.size()>0){
				continue;
			}
			Date interestBeginTime = l.getInterestBeginTime();
			//如果起息时间等于空，取放款时间与当前时间比较，相等就发送
			if(interestBeginTime==null){
				interestBeginTime = l.getGiveMoneyTime();
			}else if(DateUtil.DateToString(interestBeginTime,"yyyy-MM-dd")!=null&&DateUtil.DateToString(interestBeginTime,"yyyy-MM-dd").compareTo(now)==0){
				//如果起息时间不等于空，并且等于当前时间，就发送
				
			}else if(DateUtil.DateToString(interestBeginTime,"yyyy-MM-dd")!=null&&DateUtil.DateToString(interestBeginTime,"yyyy-MM-dd").compareTo(now)<0&&DateUtil.DateToString(interestBeginTime,"yyyy-MM-dd").compareTo(DateUtil.DateToString(l.getGiveMoneyTime(),"yyyy-MM-dd"))<0){
				//如果起息时间不等于空，并且小于当前时间，并且放款时间不等于空，取执行时间与当前时间比较，相等就发送
				interestBeginTime = l.getExpectTime();
			}else{
				//其他不发送
				log.debug("=========================没有可以计息的标===========================");
				continue;
			}
			List<String> is = ht.find("select distinct i.user.id from Invest i where i.loan.id =?",l.getId());
			for(String i : is) {
					User u = ht.get(User.class,i);
					Map<String, String> params = new HashMap<String, String>();
					params.put("username", u.getMobileNumber());
					params.put("time",DateUtil.DateToString(interestBeginTime, "yyyy年MM月dd日"));
					UserMessageTemplate umt = ht.get(UserMessageTemplate.class,MessageConstants.UserMessageNodeId.START_INTEREST_ALERT + "_sms");
					StartInterestAlertCount s = new StartInterestAlertCount();
					s.setId(IdGenerator.randomUUID());
					s.setLoanId(l.getId());
					s.setUserId(u.getId());
					s.setTime(new Date());
					s.setRmk1(replaceParams(umt,params));
					try {
					...
					List<UserMessage> uMess = ht.find("from UserMessage u where u.user.id=? and u.userMessageTemplate.id=?",i,"start_interest_alert");
					if(uMess!=null&&uMess.size()>0){
