...
//				"(loan.deadline <= #{loanList.maxDeadline} and loan.repayPeriod = 'month' or loan.deadline <= #{loanList.maxDeadline2*30} and loan.repayPeriod = 'day')",
				"attr.id = #{loanList.example.recommend}",
		...
		buffer.append("loan.id like #{loanList.example.id},loan.repayType like #{loanList.example.repayType},loan.status like #{loanList.example.status},");
		buffer.append("loan.name like #{loanList.example.name},loan.rate >=#{loanList.minRate},loan.rate <=#{loanList.maxRate},loan.status like #{loanList.example.status},");
		buffer.append("loan.riskLevel like #{loanList.example.riskLevel},loan.type like #{loanList.example.type},loan.user.id = #{loanList.example.user.id},");
		buffer.append("loan.user.username like #{loanList.example.user.username},loan.businessType like #{loanList.example.businessType},");
		if (false) {
			buffer.append("attr.id = 'recommend',");
		}
		buffer.append("loan.money >= #{loanList.minMoney},loan.money <= #{loanList.maxMoney},loan.loanMoney >= #{loanList.minLoanMoney},loan.loanMoney <= #{loanList.maxLoanMoney},");
		buffer.append("loan.commitTime >= #{loanList.searchCommitMinTime},loan.commitTime <= #{loanList.searchCommitMaxTime},loan.loanPurpose like #{loanList.example.loanPurpose}");
		String[] strArr = buffer.toString().split(",");
		setRestrictionExpressionStrings(Arrays.asList(strArr));*/
	 ...
	 * Zhangwq20150806
	 */
	public void loanRecommendList() {
		setCountHql(lazyModelCountHql+" where attr.id='recommend' ");
		setHql(lazyModelHql+" where attr.id='recommend' ");
		final String[] RESTRICTIONS = { "loan.id like #{loanList.example.id}",
				"loan.repayType like #{loanList.example.repayType}",
				"loan.status like #{loanList.example.status}",
				"loan.name like #{loanList.example.name}",
