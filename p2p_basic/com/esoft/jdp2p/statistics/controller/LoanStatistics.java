...
public class LoanStatistics {
	 ...
	 * @return
	 */
	public Long getLoanRSCountByAttr(String attrId) {
		String hql = "select count(loan) from Loan loan" +
				" left join loan.loanAttrs attr where loan.status in(?,?,?,?) and attr.id =  ?";
		/*List<Object> oos = ht.find(hql, new String[] { LoanStatus.BAD_DEBT,
				LoanStatus.COMPLETE, LoanStatus.OVERDUE, LoanStatus.RAISING,
				LoanStatus.RECHECK, LoanStatus.REPAYING,
				LoanStatus.WAITING_RECHECK_VERIFY, attrId });*/
		List<Object> oos = ht.find(hql, new String[] { 
				LoanStatus.COMPLETE,  LoanStatus.RAISING,
				LoanStatus.RECHECK, LoanStatus.REPAYING,
				 attrId });
	...
	//LoanStatus.COMPLETE;LoanStatus.RAISING ;LoanStatus.RECHECK;LoanStatus.REPAYING;
	 ...
	 * @return
	 */
	public Long getLoanCountByAttr(String loanStatus, String attrId) {
		String hql = "select count(loan) from Loan loan left join loan.loanAttrs attr" +
				" where loan.status=? and attr.id =  ?";
		List<Object> oos = ht.find(hql, new String[]{loanStatus, attrId});
		Object o = oos.get(0);
		if (o == null) {
			return 0L;
		}
