...
public class RepayServiceImpl implements RepayService {
	...
	public void normalRepay(LoanRepay repay) throws InsufficientBalance,
		...
		for (InvestRepay ir : irs) {
			...
			systemBillService.transferInto(ir.getFee(),
					OperatorInfo.NORMAL_REPAY, "投资：" + ir.getInvest().getId()
							+ "收到还款，扣除手续费, 用户ID:"+ir.getInvest().getUser().getId()+"还款ID:" + repay.getId());

			log.debug("收到还款，扣除手续费, 用户ID:"+ir.getInvest().getUser().getId()+"还款ID:" + repay.getId());
		...
		systemBillService.transferInto(repay.getFee(),
				OperatorInfo.NORMAL_REPAY, "借款：" + repay.getLoan().getId()
						+ "正常还款，扣除手续费， 用户ID:"+repay.getLoan().getUser().getId()+"还款ID：" + repay.getId());
		log.debug("正常还款，扣除手续费， 用户ID:"+repay.getLoan().getUser().getId()+"还款ID：" + repay.getId());
		ht.merge(repay);
		...
		for (InvestRepay ir : irs) {
			...
			try {
				enableRepayAlert = configService.getConfigValue(ConfigConstants.Schedule.REPAYMENT_MESSAGE_REMIND);
			} catch (ObjectNotFoundException onfe) {
				onfe.printStackTrace();
			}
			List<UserMessage> uMess = ht.find("from UserMessage u where u.user.id=? and u.userMessageTemplate.id=?",ir.getInvest().getUser().getId(),ConfigConstants.Schedule.REPAYMENT_MESSAGE_REMIND_ALERT + "_sms");
			//获取当前项目借款人
			//Loan loan = ht.get(Loan.class, repay.getLoan().getId());
			//如果放款短信为1开启则运行
			if (enableRepayAlert.equals("1")&&uMess!=null&&uMess.size()>0) {
				Map<String, String> params = new HashMap<String, String>();
				params.put("username", ir.getInvest().getUser().getMobileNumber());
				//Double aa=ArithUtil.add(repay.getCorpus(), repay.getInterest(),repay.getFee());
				//System.out.println(aa);
				params.put("money",String.valueOf(ir.getCorpus()+ir.getInterest()));
				UserMessageTemplate umt=ht.get(UserMessageTemplate.class,ConfigConstants.Schedule.REPAYMENT_MESSAGE_REMIND_ALERT + "_sms");
				//发送短信
				messageBO.sendSMS(umt,params, ir.getInvest().getUser().getMobileNumber());
			}
	...
	public void advanceRepay(String loanId) throws InsufficientBalance,
		...
		if (loan.getStatus().equals(LoanStatus.REPAYING)
			...
			/*double feeToSystem = feeConfigBO.getFee(
					...
					sumCorpus);*/
			Double nextRepayInterest = loan.getRepayRoadmap().getNextRepayInterest();
			double feeToSystem = nextRepayInterest;

			double sumPay = ArithUtil.add(feeToSystem,
					sumCorpus, feeAll);
			// 扣除还款金额+罚金
			userBillBO.transferOutFromBalance(loan.getUser().getId(), sumPay,
					OperatorInfo.ADVANCE_REPAY, "借款：" + loan.getId()
							+ "提前还款，本金：" + sumCorpus + "，用户罚金：" 
			...
			//double feeToInvestorTemp = 0; //feeToInvestor;
			// 更改投资的还款信息
			for (int i = 0; i < irs.size(); i++) {
				...
				if (enableRepayAlert.equals("1")&&uMess!=null&&uMess.size()>0) {
					...
					//Double aa=ArithUtil.add(repay.getCorpus(), repay.getInterest(),repay.getFee());
					//System.out.println(aa);
					params.put("money",String.valueOf(ir.getCorpus()+cashFine));
					UserMessageTemplate umt=ht.get(UserMessageTemplate.class,ConfigConstants.Schedule.REPAYMENT_MESSAGE_REMIND_ALERT + "_sms");
					//发送短信
					messageBO.sendSMS(umt,params, ir.getInvest().getUser().getMobileNumber());
				}
			...
			systemBillService.transferInto(ArithUtil.add(feeToSystem, feeAll),
					...
					"提前还款罚金及借款手续费到账，用户ID："+loan.getUser().getId()+"借款ID:" + loan.getId());
			log.debug("提前还款罚金及借款手续费到账，用户ID："+loan.getUser().getId()+"借款ID:" + loan.getId());
			// 改项目状态。
	...
	public void overdueRepay(String repayId) throws InsufficientBalance,
		...
		if (lr.getStatus().equals(LoanConstants.RepayStatus.OVERDUE)
			...
			for (InvestRepay ir : irs) {
				...
				systemBillService.transferInto(ir.getFee(),
						OperatorInfo.OVERDUE_REPAY, "投资："
								+ ir.getInvest().getId() + "收到还款，扣除手续费,用户ID:"+ir.getInvest().getUser().getId()+" 还款ID:"
								+ lr.getId());
				log.debug("投资："+ ir.getInvest().getId() + "收到还款，扣除手续费,用户ID:"+ir.getInvest().getUser().getId()+" 还款ID:"+ lr.getId());
				...
				if (enableRepayAlert.equals("1")&&uMess!=null&&uMess.size()>0) {
					...
					//Double aa=ArithUtil.add(repay.getCorpus(), repay.getInterest(),repay.getFee());
					//System.out.println(aa);
					params.put("money",String.valueOf(ir.getCorpus()+ir.getInterest()+ir.getDefaultInterest()));
					UserMessageTemplate umt=ht.get(UserMessageTemplate.class,ConfigConstants.Schedule.REPAYMENT_MESSAGE_REMIND_ALERT + "_sms");
					//发送短信
					messageBO.sendSMS(umt,params, ir.getInvest().getUser().getMobileNumber());
				}
			...
			systemBillService.transferInto(defaultInterest,
					OperatorInfo.OVERDUE_REPAY, "借款：" + lr.getLoan().getId()
							+ "逾期还款，扣除罚金，用户ID:"+lr.getLoan().getUser().getId()+" 还款ID：" + lr.getId());
			log.debug("借款：" + lr.getLoan().getId()+ "逾期还款，扣除罚金，用户ID:"+lr.getLoan().getUser().getId()+" 还款ID：" + lr.getId());
			ht.merge(lr);
			Long count = (Long) ht
	...
	public void repayAlert() {
		...
		for (LoanRepay lr : lrs) {
			Map<String, String> params = new HashMap<String, String>();
			params.put("username", lr.getLoan().getUser().getMobileNumber());
